...
def verify_ae_metadata(depth_sensor, expected_ae_enabled, num_frames=10):
    ...
    if depth_profile is None:
        log.error("No 30fps depth profile with z16 format found. Test requires 30fps.")
        return 0, 0
    ...
    if len(received_frames) < num_frames:
        log.warning(f"Only received {len(received_frames)} frames out of {num_frames} requested")
    ...
    for frame in received_frames[:num_frames]:
        ...
        if frame.supports_frame_metadata(rs.frame_metadata_value.auto_exposure):
            ...
            if ae_enabled_in_metadata == expected_ae_enabled:
                ...
                log.debug(f"Frame {frames_checked}: AUTO_EXPOSURE metadata = {ae_metadata}, expected AE enabled = {expected_ae_enabled}")
        else:
            log.warning(f"Frame {frames_checked} does not support AUTO_EXPOSURE metadata")
    ...
    for i in range(start_frame, end_frame):
        if i < len(context.frame_ae_metadata) and context.frame_ae_metadata[i] is not None:
            ...
            if ae_enabled_in_metadata != expected_ae_state:
                metadata_errors += 1
                log.debug(f"Frame {i}: AE metadata mismatch - expected {expected_ae_state}, metadata shows {ae_enabled_in_metadata}")
...
def wait_for_frames(context, target_frames, timeout=10.0):
    ...
    while (context.frame_count - frames_before) < target_frames:
        ...
        if elapsed > timeout:
            ...
            log.error(f"Timeout waiting for {target_frames} frames")
            log.error(f"Waited {elapsed:.1f}s but only got {frames_received}/{target_frames} frames")
            return False
        time.sleep(0.01)
...
def toggle_ae_while_streaming(depth_sensor, num_toggles=5, frames_per_state=10, frames_between_toggles=30):
    ...
    if depth_profile is None:
        log.error("No 30fps depth profile found for rapid AE toggle test")
        return False, 0, 0
    ...
    time.sleep(1.0)
    log.debug(f"Initial streaming started, received {context.frame_count} frames")
    ...
    try:
        for toggle_idx in range(num_toggles):
            ...
            if toggle_idx > 0:
                ...
                depth_sensor.set_option(rs.option.enable_auto_exposure, current_ae_state)
                toggle_count += 1
                log.debug(f"Toggled AE to {current_ae_state} (toggle {toggle_count}/{num_toggles})")
                ...
                log.debug(f"Waiting {frames_between_toggles} frames for camera to stabilize after toggle...")
                if not wait_for_frames(context, frames_between_toggles):
                    log.error(f"Failed to collect {frames_between_toggles} stabilization frames after toggling to AE={current_ae_state}")
                    try:
                        depth_sensor.stop()
                        depth_sensor.close()
                    except RuntimeError:
                ...
                log.debug(f"Collected {context.frame_count - frames_before_stabilize} stabilization frames")

            # Now wait for frames in current state
            frames_before = context.frame_count
            log.debug(f"Toggle {toggle_idx}: waiting for {frames_per_state} frames with AE={current_ae_state}, currently have {context.frame_count} total")

            if not wait_for_frames(context, frames_per_state):
                log.error(f"Failed in AE state {current_ae_state} (toggle {toggle_idx})")
                log.error(f"Total frames collected: {context.frame_count} (expected {num_toggles * frames_per_state})")
                try:
                    depth_sensor.stop()
                    depth_sensor.close()
                except RuntimeError:
            ...
            log.debug(f"Toggle {toggle_idx}: collected {context.frame_count - frames_before} frames, total now {context.frame_count}")
    ...
    log.info(f"Received {context.frame_count} total frames, metadata errors: {metadata_errors}")
...
def test_ae_metadata_with_ae_enabled(test_device):
    ...
    log.debug(f"AE enabled (default mode): {matches}/{total} frames had correct AUTO_EXPOSURE metadata")
    check.is_true(matches == total, f"Expected all {total} frames to have AUTO_EXPOSURE metadata != 0, got {matches}")
    ...
    if ae_mode_supported:
        ...
        log.debug(f"AE enabled (ACCELERATED mode): {matches_accel}/{total_accel} frames had correct AUTO_EXPOSURE metadata")
        check.is_true(matches_accel == total_accel, f"Expected all {total_accel} frames to have AUTO_EXPOSURE metadata != 0 with ACCELERATED mode, got {matches_accel}")


def test_ae_metadata_with_ae_disabled(test_device):
    ...
    log.debug(f"AE disabled: {matches}/{total} frames had correct AUTO_EXPOSURE metadata")
    check.is_true(matches == total, f"Expected all {total} frames to have AUTO_EXPOSURE metadata == 0, got {matches}")


def test_rapid_ae_toggle_metadata_correctness(test_device):
    ...
    check.is_true(success, "Camera should handle rapid AE toggles without stalling (default mode)")
    check.is_true(total_frames >= NUM_TOGGLES * FRAMES_PER_STATE, f"Expected at least {NUM_TOGGLES * FRAMES_PER_STATE} frames during rapid toggle, got {total_frames}")
    check.is_true(metadata_errors == 0, f"AUTO_EXPOSURE metadata should match enable_auto_exposure state, got {metadata_errors} mismatches")
    ...
    if ae_mode_supported:
        ...
        check.is_true(success_accel, "Camera should handle rapid AE toggles without stalling (ACCELERATED mode)")
        check.is_true(total_frames_accel >= NUM_TOGGLES * FRAMES_PER_STATE, f"Expected at least {NUM_TOGGLES * FRAMES_PER_STATE} frames during rapid toggle with ACCELERATED mode, got {total_frames_accel}")
        check.is_true(metadata_errors_accel == 0, f"AUTO_EXPOSURE metadata should match enable_auto_exposure state with ACCELERATED mode, got {metadata_errors_accel} mismatches")
