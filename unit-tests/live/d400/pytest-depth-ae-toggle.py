...
def test_baseline_streaming_measure_frame_gap(test_device):
    ...
    if not wait_for_frames(context, target_frames, timeout=20.0):
        log.error(f"Failed to collect {target_frames} baseline frames")
        try:
            depth_sensor.stop()
            depth_sensor.close()
        except RuntimeError:
            pass
        assert False, "Baseline streaming should collect frames without timeout"
    else:
        # Stop streaming
        try:
            depth_sensor.stop()
        ...
        log.info(f"Baseline: received {context.frame_count} frames, avg frame time: {baseline_avg_frame_time_ms:.2f}ms (expected: {frame_time_ms:.2f}ms), "
...
def test_rapid_ae_toggle_camera_stability(test_device):
    ...
    check.is_true(success, "Camera should handle rapid AE toggles without stalling")
    check.is_true(total_frames >= 100, f"Expected at least 100 frames during rapid toggle, got {total_frames}")
    # Expect no metadata mismatches: the helper waits after each toggle before checking.
    check.is_true(metadata_errors == 0, f"Metadata errors should be minimal during rapid toggle, got {metadata_errors}")
...
def test_switch_manual_to_auto_exposure(test_device):
    ...
    try:
        ...
        log.debug(f"Stream FPS: {fps}, frame time: {frame_time_us:.1f} us")
        ...
        for iteration in range(num_iterations):
            ...
            log.debug(f"Set manual exposure to {long_exposure:.1f} us (frame time: {frame_time_us:.1f} us)")

            # Wait and collect frames with long exposure
            time.sleep(1.5)
            frames_with_manual = context.frame_count
            log.debug(f"Iteration {iteration + 1}: received {frames_with_manual} total frames")

            # Now switch to auto exposure
            frames_before_switch = context.frame_count
            depth_sensor.set_option(rs.option.enable_auto_exposure, 1)
            log.debug(f"Iteration {iteration + 1}: Switched to auto exposure mode at frame {frames_before_switch}")

            # Wait for AE to settle and camera to recover
            time.sleep(1.5)

            frames_after_switch = context.frame_count - frames_before_switch
            log.debug(f"Iteration {iteration + 1}: received {frames_after_switch} frames after switching to AE")
            ...
            log.debug(f"Iteration {iteration + 1}: {iteration_spike_count} spikes out of {iteration_gaps} gaps")
        ...
        check.is_true(context.frame_count > 0, f"Should receive frames, got {context.frame_count}")
