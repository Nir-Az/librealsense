          ...
          <div className="p-3">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <h4 className="section-label">Streams</h4>
                {/* Streaming mode indicator */}
                {streamingMode !== 'idle' && (
                  <span className={`text-[11px] px-1.5 py-0.5 rounded border ${
                    streamingMode === 'pipeline'
                      ? 'border-rs-blue/40 bg-rs-blue/15 text-rs-accent'
                      : 'border-rs-border bg-rs-border/40 text-rs-muted'
                  }`}>
  ...
  return (
    <div className="space-y-3">
      {sensors.map((sensor) => {
        ...
        return (
          <div key={sensor.sensor_id} className="bg-rs-inset/70 border border-rs-border/60 rounded-lg p-2">
            {/* Sensor header with per-sensor start button */}
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <span className="text-sm font-semibold text-rs-text">{sensor.name}</span>
                {isSensorStreaming && (
...
function SensorOptionsPanel({ sensor, options, searchQuery, isExpanded, onToggle, onSetOption }: SensorOptionsPanelProps) {
  ...
  return (
    <div className="mb-1">
      ...
      >
        <span className="font-semibold text-rs-text">{sensor.name}</span>
        <div className="flex items-center gap-1 text-rs-muted">
          {modifiedCount > 0 && (
