            ...
            for( auto && sub : dev.query_sensors() )
            {
                    ...
                    && sub.supports( RS2_CAMERA_INFO_PRODUCT_LINE ) )
                {
                    ...
                    std::string recommended
                        = sub.get_info( RS2_CAMERA_INFO_RECOMMENDED_FIRMWARE_VERSION );

                    int product_line
                        = parse_product_line( sub.get_info( RS2_CAMERA_INFO_PRODUCT_LINE ) );
    ...
    void device_model::check_for_device_updates(viewer_model& viewer)
    {
        ...
        std::thread check_for_device_updates_thread( [ctx,
            ...
            try
            {
                ...
                if((bool)config_file::instance().get(configurations::update::recommend_updates) && need_to_check_bundle )
                {
                    if( auto nm = notification_model_protected.lock() )
                    {
